...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZtstStake(spend));
    } else {
        ...
        CtstStake* tstInput = new CtstStake();
        tstInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(tstInput);
    }
