...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 tst.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
static void NotifyztstReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyztstReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyztstReset.connect(boost::bind(NotifyztstReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyztstReset.disconnect(boost::bind(NotifyztstReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->ztstTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
