...
#include "ui_ztstcontroldialog.h"
...
std::set<std::string> ZtstControlDialog::setSelectedMints;
std::set<CMintMeta> ZtstControlDialog::setMints;

bool CZtstControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZtstControlDialog::COLUMN_DENOMINATION || column == ZtstControlDialog::COLUMN_VERSION || column == ZtstControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZtstControlDialog::ZtstControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZtstControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZtstControlDialog::~ZtstControlDialog()
{
    delete ui;
}

void ZtstControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZtstControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZtstControlWidgetItem* itemDenom(new CZtstControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZtstControlWidgetItem *itemMint = new CZtstControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
...
void ZtstControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZtstControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZtst_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZtstControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZtstControlDialog::GetSelectedMints()
{
...
void ZtstControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
