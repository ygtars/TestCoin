...
public:
    ...
    bool WriteZtstSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZtstSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZtstSeed_deprecated(uint256& seed);
    bool EraseZtstSeed();
    bool EraseZtstSeed_deprecated();

    bool WriteZtstCount(const uint32_t& nCount);
    bool ReadZtstCount(uint32_t& nCount);
