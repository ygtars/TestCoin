// Copyright (c) 2018 The test developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <primitives/deterministicmint.h>
#include "ztsttracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CztstTracker::CztstTracker(std::string strWalletFile)
{
...
CztstTracker::~CztstTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CztstTracker::Init()
{
...
bool CztstTracker::Archive(CMintMeta& meta)
{
...
bool CztstTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CztstTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CztstTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CztstTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CztstTracker::GetSerialHashes()
{
...
CAmount CztstTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CztstTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CztstTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CztstTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CztstTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CztstTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CztstTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CztstTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CztstTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CztstTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CztstTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CztstTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CztstTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CztstTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CztstTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CztstTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CztstTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dztst from DB\n", __func__, listDeterministicDB.size());
    }
