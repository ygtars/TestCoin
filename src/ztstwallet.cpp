...
CztstWallet::CztstWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing ztst seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZtstSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZtstSeed_deprecated()) {
                    LogPrintf("%s: Updated ztst seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old ztst seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of ztst wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CztstWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZtstCount(nCountLastUsed);
    else if (!walletdb.ReadZtstCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CztstWallet::Lock()
{
    seedMaster = 0;
}

void CztstWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CztstWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZtst(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CztstWallet::LoadMintPoolFromDB()
{
...
void CztstWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CztstWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CztstWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->ztstTracker->HasPubcoinHash(pMint.first)) {
...
bool CztstWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZtst(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to ztstTracker which also adds to database
    pwalletMain->ztstTracker->Add(dMint, true);
...
void CztstWallet::SeedToZtst(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CztstWallet::GetZerocoinSeed(uint32_t n)
{
...
void CztstWallet::GenerateDeterministicZtst(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CztstWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CztstWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
