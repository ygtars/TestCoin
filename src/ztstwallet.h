...
#ifndef test_ZtstWALLET_H
#define test_ZtstWALLET_H
...
public:
    CztstWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZtst(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZtst(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //test_ZtstWALLET_H
